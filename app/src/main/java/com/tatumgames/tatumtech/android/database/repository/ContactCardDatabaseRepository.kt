...
class ContactCardDatabaseRepository(
    private val contactCardDao: ContactCardDao
) {
    suspend fun getByOwnerUserId(ownerUserId: Long): ContactCardEntity? =
        contactCardDao.getByOwnerUserId(ownerUserId)

    fun observeByOwnerUserId(ownerUserId: Long): Flow<ContactCardEntity?> =
        contactCardDao.observeByOwnerUserId(ownerUserId)

    suspend fun getByCardId(cardId: String): ContactCardEntity? =
        contactCardDao.getByCardId(cardId)

    suspend fun upsert(card: ContactCardEntity) = contactCardDao.upsert(card)
}
